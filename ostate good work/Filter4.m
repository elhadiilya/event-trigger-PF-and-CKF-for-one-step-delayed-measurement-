...
function [xhat,gamt]= Filter4(n, ny, Q, R, M, phi, gam, xp3, pp3, kmax, xi1, p, z,del);
% xi1=Z.xi1; z=Z.z;
...
for k = 1:m.kmax-1
     ...
     %% - one step delay measurement-----------------------------
      ybarc(k+1) = (1-xi1(k+1))*zbarc(k+1) + xi1(k+1)*zbarc(k);
     %% - Measurement update -----------------------------------
    weights = gamt(k+1)*((1-p)*likelihood(ybarc( k+1),Mesparticle,m.R)+ (p)*likelihood(ybarc( k+1),prouviosMesparticle,m.R))...
            +(1-gamt(k+1))*((1-p)*BayesianConstraint(ybarc( k+1),Mesparticle,m.R,del)+ (p)*BayesianConstraint(ybarc( k+1),prouviosMesparticle,m.R,del));
    if sum(weights) ==0
